...
from eventyst._exceptions import SchemaRegistryException
from eventyst.schema import SchemaFormat, SchemaVersion
from eventyst.serialization.registry import SchemaCompatibilityMode, SchemaRegistryClient
...
class GlueSchemaRegistryClient(SchemaRegistryClient):
    """Schema registry for AWS Glue Schema Registry."""
    ...
    def get_schema_version(self, version_id: UUID) -> SchemaVersion:
        ...
        """
        try:
            res = self.glue_client.get_schema_version(SchemaVersionId=str(version_id))
        except Exception as e:
            raise SchemaRegistryException(f'Failed to get schema version by id {version_id}') from e
        if res['SchemaVersionId'] is None or res['Status'] != 'AVAILABLE':
            raise SchemaRegistryException(f"Schema Found but status is {res['Status']}")
    ...
    def get_schema_by_definition(self, definition: str, schema_name: str) -> SchemaVersion:
        ...
        """
        try:
            logger.debug('Getting schema version id for: name = %s, definition = %s', schema_name, definition)
            ...
            raise SchemaRegistryException(
                'Failed to get schemaVersionId by schema definition for schema' f' name = {schema_name}'
            ) from e
    ...
    def get_or_register_schema_version(
        ...
        compatibility_mode: SchemaCompatibilityMode = DEFAULT_COMPATIBILITY_MODE,
        ...
        """
        try:
            version = self.get_schema_by_definition(definition, schema_name)
        except SchemaRegistryException as e:
            cause_msg = str(e.__cause__)
            if SCHEMA_VERSION_NOT_FOUND_MSG in cause_msg:
                logger.debug(cause_msg)
                version_id = self.register_schema_version(definition, schema_name, metadata)
            elif SCHEMA_NOT_FOUND_MSG in cause_msg:
                logger.debug(cause_msg)
                version_id = self.create_schema(schema_name, data_format, definition, compatibility_mode, metadata)
            else:
                raise SchemaRegistryException(
    ...
    def register_schema_version(self, definition: str, schema_name: str, metadata: Mapping[str, str] = None) -> UUID:
    ...
    def _wait_for_schema_evolution_check_to_complete(self, schema_version_id: UUID):
        time.sleep(self.wait_interval_seconds)
        for _ in range(self.max_wait_attempts):
            res = self.glue_client.get_schema_version(SchemaVersionId=str(schema_version_id))
            status = res['Status']
            if status == 'AVAILABLE':
                break
            elif status != 'PENDING':
                raise SchemaRegistryException(
                    'Schema evolution check failed.' f' schemaVersionId {schema_version_id} is in' f' {status} status.'
                )
        else:
            raise SchemaRegistryException(
                'Retries exhausted for schema evolution check for ' f'schemaVersionId = {schema_version_id}'
            )

    def put_schema_version_metadata(self, version_id: UUID, metadata: Mapping[str, str]):
        for k, v in metadata.items():
            try:
                self.glue_client.put_schema_version_metadata(
                    SchemaVersionId=str(version_id), MetadataKeyValue={'MetadataKey': k, 'MetadataValue': v}
                )
            except Exception as e:
                raise SchemaRegistryException(
    ...
    def create_schema(
        ...
        try:
            ...
            if type(e).__name__ == 'AlreadyExistsException':
                logger.warning(
                    'Schema is already created, this could be caused by '
                    'multiple producers racing to auto-create schema.'
                )
                version_id = self.register_schema_version(definition, name, metadata)
            else:
                raise SchemaRegistryException(f'Create schema {name} failed') from e
        return version_id
