...
def encode(data: bytes, schema_version_id: UUID, compression=None) -> bytes:
...
def decode(bytes_: bytes) -> tuple[bytes, UUID]:
    ...
    if version != VERSION_BYTE:
        raise UnknownEncodingException(r"leading byte {version!r} not recognized")
    ...
    if compression == COMPRESSION_ENABLED_BYTE:
        ...
        raise CodecException(f'compression byte {compression!r} not recognized')
